// Llantas Total — Order Follow-up: recupera clientes que dejaron el pedido a medias
//
...
const PRODUCT_NAMES = {
  'llantas-vinmax': 'el juego de llantas VINMAX ECOTOUR HP3'
};
...
function detectProduct(t) {
  ...
  if (/llantas?\b|neum[a\u00e1]tic|vinmax|ecotour|\bhp\s*3\b|\b\d{3}\s*\/\s*\d{2}\s*r?\s*\d{2}\b|\brin(es)?\s*\d{2}\b/.test(t)) return 'llantas-vinmax';
  return null;
}
